...
const useDeviceSize = () => {
  ...
  const [mainWidth, setMainWidth] = useState(get16x9Ratio(Dimensions.get("window").height, Dimensions.get("window").width).width);
  const [mainHeight, setMainHeight] = useState(get16x9Ratio(Dimensions.get("window").height, Dimensions.get("window").width).height);
